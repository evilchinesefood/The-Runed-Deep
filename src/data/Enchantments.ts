...
// effective = base * enchantment (no caps)
// ============================================================
...
export interface Affix {
  ...
  description: string;
  color: string;
  base: number; // multiplied by enchantment level
...
export const AFFIXES: Affix[] = [
  ...
  },
  {
    id: "thorns",
    name: "Thorns",
    description: "{v}% damage reflected",
    color: "#f84",
    base: 0.5,
    weight: 1,
  },
  {
    id: "fire-touched",
    name: "Fire Touched",
    description: "+{v}% fire damage, +{v2} fire resist",
    color: "#f64",
    base: 1,
    base2: 1,
    weight: 1,
  },
  {
    id: "frost-touched",
    name: "Frost Touched",
    description: "+{v}% cold damage, +{v2} cold resist",
    color: "#6cf",
    base: 1,
    base2: 1,
    weight: 1,
  },
  {
    id: "storm-touched",
    name: "Storm Touched",
    description: "+{v}% lightning damage, +{v2} lightning resist",
    color: "#ff8",
    base: 1,
    base2: 1,
    weight: 1,
  ...
  },
  {
    ...
    description: "+{v}% all elemental resist",
    color: "#a8f",
    base: 0.25,
    weight: 1,
  },
  ...
  },
  {
    id: "regeneration",
    name: "Regeneration",
    description: "+{v} HP every 2 turns",
    color: "#4f4",
    base: 0.25,
    weight: 1,
  },
  ...
  },
  {
    id: "swiftness",
    name: "Swiftness",
    description: "{v}% chance for extra action",
    color: "#fc4",
    base: 0.25,
    weight: 1,
  },
  ...
  },
  {
    id: "arcane-mastery",
    name: "Arcane Mastery",
    description: "-{v}% MP cost, +{v2} MP/3 turns",
    color: "#a6f",
    base: 0.25,
    base2: 0.25,
    weight: 1,
  ...
  // ── Cursed-only affixes (penalty scales at half the benefit rate) ──
  {
    id: "blood-price",
    name: "Blood Price",
    description: "+{v}% damage, lose {v2} HP per hit",
    color: "#d44",
    base: 1,
    base2: 0.5,
    cursedOnly: true,
  ...
  },
  {
    id: "soul-drain",
    name: "Soul Drain",
    description: "+{v} to all stats, -{v2} max HP",
    color: "#a4d",
    base: 1,
    base2: 0.5,
    cursedOnly: true,
  ...
  },
  {
    id: "dark-pact",
    name: "Dark Pact",
    description: "+{v}% spell damage, +{v2}% MP cost",
    color: "#84f",
    base: 1,
    base2: 0.5,
    cursedOnly: true,
  ...
  },
  {
    id: "berserk-fury",
    name: "Berserk Fury",
    description: "+{v}% melee damage, +{v2}% damage taken",
    color: "#f66",
    base: 1,
    base2: 0.5,
    cursedOnly: true,
  ...
  },
  {
    id: "leech",
    name: "Leech",
    description: "Heal {v}% of damage, -{v2}% XP gained",
    color: "#6a4",
    base: 1,
    base2: 0.5,
    cursedOnly: true,
...
export function getAffixValue(
  ...
  return affix.base * Math.max(1, enchantLevel) * mult;
}

/** Compute the secondary value (for dual-stat affixes like Touched) */
export function getAffixValue2(
  affixId: string,
  enchantLevel: number,
  critical: boolean,
): number {
  ...
  const mult = critical ? 2 : 1;
  return affix.base2 * Math.max(1, enchantLevel) * mult;
}

/** Format affix description with computed values */
export function formatAffixDesc(
  affixId: string,
  enchantLevel: number,
  critical: boolean,
): string {
...
export function getEnchantStrength(
...
export function getEquipEnchantTotal(
 ...
 */
export function getEquipAffixTotal(
  equipment: Record<string, any>,
  affixId: string,
...
export function getEquipAffixTotal2(
  equipment: Record<string, any>,
  affixId: string,
  statueUpgrades?: Record<string, number>,
): number {
...
export function rollSpecialEnchantments(
  ...
  if (isTierItem) {
    ...
    chance = Math.min(0.7, 0.25 + Math.max(0, depth - 10) * 0.01 + ngBonus);
    baseCount = 2;
    ...
    chance =
      depth >= 25
        ? Math.min(0.2, 0.02 + (depth - 25) * 0.01 + ngBonus)
        : ngBonus > 0
          ? Math.min(0.1, ngBonus)
          : 0;
    baseCount = 1;
  }
  ...
  const maxCount =
    !isTierItem && (enchantment > 0 || isCursed) && ngPlus === 0
      ? 3
      : 5 + ngPlus * 2;
  ...
  const critChance =
    ngPlus <= 0 ? 0 : ngPlus === 1 ? 0.2 : ngPlus === 2 ? 0.3 : 0.4;
  if (critChance > 0) {
    return result.map((id) =>
      Math.random() < critChance ? `${id}:critical` : id,
    );
  }
