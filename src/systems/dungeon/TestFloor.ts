...
let monsterId = 1;
function makeMonster(templateId: string, name: string, sprite: string, x: number, y: number, hp: number, dmg: [number, number], xp: number): Monster {
  return {
    id: `test-m-${monsterId++}`,
    templateId, name, sprite,
    position: { x, y },
    hp, maxHp: hp,
    damage: dmg,
    speed: 0.5,
...
export function generateTestFloor(): { floor: Floor; playerStart: { x: number; y: number } } {
  monsterId = 1;
  ...
  for (let x = 30; x <= 37; x++) { tiles[3][x] = wall(); tiles[8][x] = wall(); }
  for (let y = 3; y <= 8; y++) { tiles[y][30] = wall(); tiles[y][37] = wall(); }
  ...
  const monsters: Monster[] = [
    ...
    makeMonster('walking-corpse', 'Walking Corpse', 'walking-corpse', 12, 15, 30, [4, 8], 30),
    ...
    makeMonster('gruesome-troll', 'Gruesome Troll', 'gruesome-troll', 20, 5, 50, [6, 12], 50),
    // Target behind wall (for LOS testing)
    makeMonster('necromancer', 'Necromancer', 'necromancer', 33, 5, 35, [5, 10], 40),
  ];

  // Items on ground — mix of types for pickup/identify testing
  const items: PlacedItem[] = [];
  const potionTpl = ALL_ITEM_TEMPLATES.find(t => t.id === 'potion-heal-minor')!;
  ...
  const removeCurseTpl = ALL_ITEM_TEMPLATES.find(t => t.id === 'scroll-remove-curse')!;

  // Normal items
  items.push({ item: createItemFromTemplate(potionTpl, 5), position: { x: 3, y: 12 } });
  items.push({ item: createItemFromTemplate(potionTpl, 5), position: { x: 3, y: 13 } });
  items.push({ item: createItemFromTemplate(scrollTpl, 5), position: { x: 3, y: 14 } });
  items.push({ item: createItemFromTemplate(removeCurseTpl, 5), position: { x: 3, y: 15 } });

  // Enchanted sword (+2)
  const enchSword: Item = {
    ...createItemFromTemplate(swordTpl, 15),
    name: 'Long Sword +2', enchantment: 2, identified: true, sprite: 'sword-enchanted',
  };
  ...
  const cursedArmor: Item = {
    ...
    name: 'Chain Mail -1', enchantment: -1, cursed: true, identified: false, sprite: 'metal-armour-cursed',
  };
  ...
  const enchRing: Item = {
    ...
    name: 'Ring +1', enchantment: 1, identified: false, sprite: 'ring-enchanted',
  };
