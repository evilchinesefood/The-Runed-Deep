...
The Castle of the Winds grows silent. You have avenged Bjarnarhaven. Your name will be sung in the halls of Odin for eternity.`);
  ...
  parchment.appendChild(statRow('Difficulty', diffLabels[state.difficulty] ?? state.difficulty));
  ...
  const scoreLbl = el('span', { color: '#6a5030', fontWeight: 'bold' }, 'Final Score');
  const scoreVal = el('span', { fontWeight: 'bold', color: '#b8860b', fontSize: '18px' }, score.toLocaleString());
  scoreRow.appendChild(scoreLbl);
  scoreRow.appendChild(scoreVal);
  parchment.appendChild(scoreRow);

  if (rank > 0) {
    parchment.appendChild(el('div', {
      textAlign: 'center',
      fontSize: '12px',
      color: '#8a7550',
      marginBottom: '16px',
  ...
  const ngPlusLabel = state.ngPlusCount > 0
  ...
  ngBtn.addEventListener('mouseenter', () => { ngBtn.style.background = '#6a5540'; });
  ngBtn.addEventListener('mouseleave', () => { ngBtn.style.background = '#5a4530'; });
  ngBtn.addEventListener('click', onNewGamePlus);
  btnRow.appendChild(ngBtn);

  const titleBtn = el('div', {
    padding: '10px 20px',
    color: '#8a7550',
    cursor: 'pointer',
    fontSize: '13px',
