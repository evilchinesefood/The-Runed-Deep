...
function iconLine(icon: string, text: string, color = "#aeb"): HTMLElement {
  return d(
    "div",
    { color, paddingLeft: "6px", fontSize: TT_SIZE },
    `${icon} ${text}`,
  );
}
...
export function setTooltipKnownSpells(spells: string[]): void {
  _knownSpells = spells;
}
...
export function buildTooltipContent(item: Item): HTMLElement {
  ...
  if (tpl?.equipSlot) {
    ...
    container.appendChild(
      d(
        "div",
        { color: "#777", fontSize: TT_SIZE, marginBottom: "4px" },
        slotText,
      ),
    );
  } else if (tpl) {
    container.appendChild(
      d(
        "div",
        { color: "#777", fontSize: TT_SIZE, marginBottom: "4px" },
        `Floors ${tpl.depthMin}\u2013${tpl.depthMax === 30 ? "30" : tpl.depthMax}`,
      ),
    );
  }
  ...
  if (tier)
    statusParts.push(
      `${tier.charAt(0).toUpperCase() + tier.slice(1)} craftsmanship`,
    );
  if (statusParts.length > 0) {
    ...
    container.appendChild(
      d(
        "div",
        { color, fontSize: TT_SIZE, marginBottom: "4px", fontStyle: "italic" },
        statusParts.join(" \u00B7 "),
      ),
    );
  }
  ...
  // Damage
  if (
    item.properties["damageMin"] !== undefined &&
    item.properties["damageMax"] !== undefined
  ) {
    ...
    const txt =
      ench !== 0
        ? `${effMin}\u2013${effMax} damage (base ${baseMin}\u2013${baseMax})`
        : `${baseMin}\u2013${baseMax} damage`;
  ...
  if (item.category === "weapon") {
    ...
    if (ench !== 0) {
      statsBox.appendChild(
        iconLine(
          "\uD83C\uDFAF",
          `${effAcc > 0 ? "+" : ""}${effAcc} accuracy (base ${baseAcc > 0 ? "+" : ""}${baseAcc})`,
        ),
      );
    } else if (baseAcc !== 0) {
      statsBox.appendChild(
  ...
  if (item.properties["healPct"]) {
    ...
    statsBox.appendChild(
      iconLine("\u2764", `Heals ${pct}% HP (min ${flat})`, "#4f4"),
    );
    hasStats = true;
  } else if (item.properties["healAmount"]) {
    statsBox.appendChild(
      iconLine("\u2764", `Heals ${item.properties["healAmount"]} HP`, "#4f4"),
    );
    hasStats = true;
  }

  // Spellbook spell
  if (item.category === "spellbook" && tpl?.spellId) {
    const spellName = tpl.spellId
      .replace(/-/g, " ")
  ...
  if (item.properties["charges"] !== undefined) {
    statsBox.appendChild(
      iconLine("\u26A1", `Charges: ${item.properties["charges"]}`),
    );
    hasStats = true;
  }
  ...
  if (item.templateId.startsWith("potion-gain-")) {
    ...
    statsBox.appendChild(
      iconLine(
        "\u2B06",
        `Permanently +1 ${attr.charAt(0).toUpperCase() + attr.slice(1)}`,
        "#4f4",
      ),
    );
    hasStats = true;
  ...
  if (enchanterUps && enchanterUps > 0) {
    container.appendChild(
      iconLine("\u25C6", `Enchanted +${enchanterUps}`, "#4af"),
    );
  }
  ...
  if (tpl?.uniqueAbility) {
    ...
    const abilityDesc: Record<string, string> = {
      "resist-fire-75": item.properties?.["wardUpgraded"]
        ? "+99 Fire Resistance"
        : "+75 Fire Resistance",
      "resist-cold-75": item.properties?.["wardUpgraded"]
        ? "+99 Cold Resistance"
        : "+75 Cold Resistance",
      "resist-lightning-75": item.properties?.["wardUpgraded"]
        ? "+99 Lightning Resistance"
        : "+75 Lightning Resistance",
      "resist-drain-75": item.properties?.["wardUpgraded"]
        ? "+99 Drain Resistance"
        : "+75 Drain Resistance, immune to level drain",
      "detect-monsters": "Reveals all monsters on the floor",
      "lightning-boost":
        "+50% Lightning spell damage, +75 Lightning Resistance",
      levitation: "Immune to pit and portal traps, walk over water",
      "elemental-immunity":
        "+50% all elemental resistances, immune to traps and poison",
...
export function showPileTooltip(items: Item[], x: number, y: number): void {
  ...
  tip.appendChild(
    d(
      ...
      `Treasure Pile (${items.length} items)`,
    ),
  );
  for (const item of items.slice(0, 8)) {
    tip.appendChild(
      d(
        "div",
        { fontSize: "12px", color: "#ccc", paddingLeft: "6px" },
        `· ${getDisplayName(item)}`,
      ),
    );
  }
  if (items.length > 8) {
    tip.appendChild(
      d(
        "div",
        { fontSize: "11px", color: "#888", paddingLeft: "6px" },
