...
export function createDeathScreen(
  ...
  tombstone.appendChild(el("div", { fontSize: "14px", color: "#666", marginBottom: "16px" }, `Level ${hero.level} · ${state.currentDungeon === "rift" ? "Fractured Rift" : state.currentDungeon === "crucible" ? "The Crucible" : `Floor ${state.currentFloor}`}`));
  ...
  tombstone.appendChild(el("div", { fontSize: "13px", color: "#777", marginBottom: "6px" }, `XP: ${hero.xp} · \$${hero.gold}`));
