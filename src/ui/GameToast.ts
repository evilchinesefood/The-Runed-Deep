...
> = {
  info: {
    border: "#48f",
    bg: "linear-gradient(135deg, #0a1a2a, #1a2a3a)",
    icon: "ℹ",
  },
  success: {
    border: "#4f4",
    bg: "linear-gradient(135deg, #0a2a0a, #1a3a1a)",
    icon: "✓",
  },
  warning: {
    border: "#fa0",
    bg: "linear-gradient(135deg, #2a1a00, #3a2a10)",
    icon: "⚠",
  },
  error: {
    border: "#f44",
    bg: "linear-gradient(135deg, #2a0a0a, #3a1a1a)",
    icon: "✗",
  },
};

function reflowToasts(): void {
  toastQueue.forEach((t, i) => {
    t.style.top = `${10 + i * 48}px`;
  });
}
...
export function showGameToast(
  ...
  toast.style.cssText = `
    position: fixed;
    top: ${10 + toastQueue.length * 48}px;
    left: 50%;
    ...
    transition: opacity 0.4s, top 0.2s ease-out;
    max-width: 400px;
  `;
